...
<template>
  <div class="finish-up-view">
    ...
    <FormDescription text="Double-check everything looks OK before confirming."></FormDescription>
    <div class="plan-addon-list">
      <div class="plan-addon-list-item">
        <div class="plan-description">
          <p class="marine-blue bold">
            {{ infoStore.selectedPlan.title }}
            {{ infoStore.selectedPlanTime === PlanEnum.Month ? '(Monthly)' : '(Yearly)' }}
          </p>
        ...
        <p class="marine-blue bold">
          ${{
            infoStore.selectedPlanTime === PlanEnum.Month
              ? infoStore.selectedPlan.pricePerMonth
              : infoStore.selectedPlan.pricePerYear
      ...
      <div class="addon-description" v-for="addon in infoStore.selectedAddons" :key="addon.title">
        ...
        <p class="marine-blue small-font">
          +${{
            infoStore.selectedPlanTime === PlanEnum.Month
              ? addon.pricePerMonth
              : addon.pricePerYear
    ...
    <div class="total">
      <p class="cool-gray">
        Total {{ infoStore.selectedPlanTime === PlanEnum.Month ? '(per month)' : '(per year)' }}
      </p>
      <p class="bold purplish-blue">
        ${{ calcTotal() }}/{{ infoStore.selectedPlanTime === PlanEnum.Month ? 'mo' : 'yr' }}
      </p>
    </div>
  </div>
</template>
